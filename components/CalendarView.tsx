...
import { Settings, Plus, X, CheckCircle, UserPlus, RefreshCw, Calendar as CalendarIcon, ExternalLink } from 'lucide-react';
import { 
...
import { openCalendarEvent, generateGoogleCalendarLink, generateOutlookCalendarLink, generateAppleCalendarICS } from '../services/calendarDeepLinks';
...
const CalendarView: React.FC<CalendarViewProps> = ({ events, currentUser, onAddEvent, onNavigate, users }) => {
    const [isAddEventOpen, setIsAddEventOpen] = useState(false);
    const [newEvent, setNewEvent] = useState({ title: '', date: '', time: '10:00', duration: '1h', type: 'Virtual', participants: [] as string[] });
    ...
    const handleSaveEvent = async () => {
       ...
       const event: Omit<CalendarEvent, 'id' | 'createdAt'> = {
           ...
           ...(newEvent.participants.length > 0 && { participants: newEvent.participants }),
       };
       ...
       setNewEvent({ title: '', date: '', time: '10:00', duration: '1h', type: 'Virtual', participants: [] });
       setShowSuccess(true);
       ...
       <div className="h-full flex flex-col space-y-6 animate-in fade-in">
          <div className="flex justify-between items-center">
              <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400">Manage your mentorship sessions and events.</p>
              </div>
              <div className="flex gap-3">
                  ...
                  <button onClick={() => setIsAddEventOpen(true)} className={BUTTON_PRIMARY}>
                                      ...
                                      <div className="space-y-1">
                                          {dayEvents.map(ev => {
                                            ...
                                            const participantNames = ev.participants
                                              ? ev.participants.slice(0, 2).map(id => {
                                                  const user = users.find(u => u.id === id);
                                                  return user ? user.name.split(' ')[0] : '';
                                                }).filter(Boolean).join(', ')
                                              : '';
                                            const extraCount = ev.participants && ev.participants.length > 2 ? ev.participants.length - 2 : 0;
                                            
                                            return (
                                              <div key={ev.id} className="text-[10px] bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-200 px-1.5 py-1 rounded truncate border border-indigo-200 dark:border-indigo-800 group relative cursor-pointer hover:z-10">
                                                  <div className="truncate">
                                                    {ev.startTime} {ev.title}
                                                  </div>
                                                  {participantNames && (
                                                    <div className="text-[9px] text-indigo-600 dark:text-indigo-300 truncate">
                                                      {participantNames}{extraCount > 0 ? ` +${extraCount}` : ''}
                                                    </div>
                                                  )}
                                                  ...
                                                  <div className="hidden group-hover:block absolute left-0 top-full mt-1 bg-slate-900 text-white text-[10px] rounded px-2 py-1 z-20 whitespace-nowrap shadow-lg min-w-[200px]">
                                                    {ev.participants && ev.participants.length > 0 && (
                                                      <div className="mb-2 pb-2 border-b border-slate-700">
                                                        Participants: {ev.participants.map(id => {
                                                          const user = users.find(u => u.id === id);
                                                    ...
                                                    <div className="space-y-1">
                                                      <div className="text-[9px] text-slate-400 mb-1">Add to calendar:</div>
                                                      <a 
                                                        href={generateGoogleCalendarLink(ev, ev.googleMeetLink)} 
                                                        target="_blank" 
                                                      ...
                                                      <a 
                                                        href={generateOutlookCalendarLink(ev, ev.googleMeetLink)} 
                                                        target="_blank" 
                                                        rel="noopener noreferrer"
                                                        className="block hover:bg-slate-800 px-1 py-0.5 rounded text-[9px]"
                                                        onClick={(e) => e.stopPropagation()}
                                                      ...
                                                      <button
                                                        onClick={(e) => {
                                                          e.stopPropagation();
                                                          const ics = generateAppleCalendarICS(ev, ev.googleMeetLink);
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in p-4">
                  <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl max-w-2xl w-full border border-slate-200 dark:border-slate-800 max-h-[90vh] flex flex-col">
                      ...
                      <div className="flex justify-between items-center p-6 border-b border-slate-200 dark:border-slate-800">
                          ...
                          <button onClick={() => { setIsAddEventOpen(false); setShowParticipantDropdown(false); }} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><X className="w-5 h-5" /></button>
                      </div>
                      
                      {/* Scrollable Content */}
                      <div className="flex-1 overflow-y-auto p-6">
                          <div className="space-y-4">
                              <div><label className="block text-xs font-medium text-slate-500 uppercase mb-1">Event Title</label><input className={INPUT_CLASS} value={newEvent.title} onChange={e => setNewEvent({...newEvent, title: e.target.value})} placeholder="e.g. Weekly Check-in" /></div>
                              <div className="grid grid-cols-2 gap-4">
                                  <div><label className="block text-xs font-medium text-slate-500 uppercase mb-1">Date</label><input type="date" className={INPUT_CLASS} value={newEvent.date} onChange={e => setNewEvent({...newEvent, date: e.target.value})} /></div>
                                  <div><label className="block text-xs font-medium text-slate-500 uppercase mb-1">Time</label><input type="time" className={INPUT_CLASS} value={newEvent.time} onChange={e => setNewEvent({...newEvent, time: e.target.value})} /></div>
                              </div>
                              <div className="grid grid-cols-2 gap-4">
                                   <div><label className="block text-xs font-medium text-slate-500 uppercase mb-1">Duration</label><select className={INPUT_CLASS} value={newEvent.duration} onChange={e => setNewEvent({...newEvent, duration: e.target.value})}><option>30 min</option><option>1h</option><option>1.5h</option></select></div>
                                  <div><label className="block text-xs font-medium text-slate-500 uppercase mb-1">Type</label><select className={INPUT_CLASS} value={newEvent.type} onChange={e => setNewEvent({...newEvent, type: e.target.value})}><option>Virtual</option><option>In-Person</option><option>Phone</option></select></div>
                              </div>
                              
                              {/* Participants Multi-Select */}
                              <div>
                                <label className="block text-xs font-medium text-slate-500 uppercase mb-1">Participants</label>
                                  ...
                                  >
                                    <span className="text-sm">
                                      {newEvent.participants.length === 0 
                                        ? 'Select participants...'
                                        : `${newEvent.participants.length} participant${newEvent.participants.length > 1 ? 's' : ''} selected`
                                      }
                                    </span>
                      ...
                      <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 rounded-b-xl">
                          <div className="flex gap-2">
                            <button onClick={handleSaveEvent} disabled={!newEvent.title || !newEvent.date} className={BUTTON_PRIMARY + " flex-1"}>Schedule Event</button>
                              ...
                              <div className="flex gap-1">
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    window.open(generateGoogleCalendarLink(tempEvent), '_blank');
                                  }}
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    window.open(generateOutlookCalendarLink(tempEvent), '_blank');
                                  }}
                                  className="px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 text-xs"
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    const link = document.createElement('a');
                                    link.href = url;
                                    link.download = `${newEvent.title.replace(/[^a-z0-9]/gi, '_')}.ics`;
                                    link.click();
                                    URL.revokeObjectURL(url);
                                  }}
                                  className="px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 text-xs"
          ...
          {showSuccess && (
             <div className="fixed bottom-6 right-6 bg-emerald-800 text-white px-4 py-3 rounded-lg shadow-lg flex items-center animate-in slide-in-from-bottom-4 fade-in z-50">
                ...
                <div>
                   <p className="font-bold text-sm">{syncing ? 'Calendar Synced' : 'Event Scheduled'}</p>
                   <p className="text-xs text-emerald-200">{syncing ? 'Events synced successfully' : 'Added to your calendar.'}</p>
                </div>
             </div>
          )}
