...
interface CityStateZipInputProps {
  value: CityStateZip;
...
// This is a simplified list - in production, you'd use a geocoding API
...
export const CityStateZipInput: React.FC<CityStateZipInputProps> = ({
  value,
  onChange,
  ...
  const filteredCities = useMemo(() => {
    if (!searchQuery.trim()) {
      return COMMON_CITIES.slice(0, 10); // Show top 10 by default
    }
          ...
          {isOpen && !disabled && (
            <div className="absolute z-50 mt-2 w-full bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800 max-h-64 overflow-y-auto">
              {filteredCities.length > 0 ? (
                <>
                  <div className="px-3 py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase border-b border-slate-200 dark:border-slate-800">
                    Suggested Locations
                  </div>
                  ...
                  <div className="px-4 py-2 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
                    <button
                      ...
                      className="text-xs text-emerald-600 dark:text-emerald-400 hover:underline w-full text-center"
                    >
                      Or enter manually →
                    </button>
                  </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="relative">
              <select
                ...
                disabled={disabled}
                required={required}
                className={`${INPUT_CLASS} ${disabled ? 'opacity-50 cursor-not-allowed' : ''} appearance-none pr-8`}
              >
              ...
              {filteredCities
                ...
                .map((city, index) => (
                  <button
                    ...
                    className="w-full px-3 py-2 text-left hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-sm border-b border-slate-100 dark:border-slate-800 last:border-b-0"
                  >
